...
**题目解法**: 该题目因为要求保证分隔后保持节点原有的位置，即将分隔的前半部分的大于`x`的节点顺序后推到后半部分，不能乱序。双指针技巧能快速进行分区(通过交快慢指针节点)，但是没法保证其节点顺序,所以我们只用另外一种方式，即将比`x`小的节点顺序插入到`left`链表中，比`x`大的节点顺序插入到`right`链表中,最后再将`right`链表追加到`left`链表后面，这样我们就完成分隔并保证节点原来的顺序。

[rust版本](../../../codes/rust/86.分隔链表.rs) |
[java版本](../../../codes/java/86.分隔链表.java) |
[golang版本](../../../codes/golang/86.分隔链表.go) |
[python版本](../../../codes/python/86.分隔链表.py)

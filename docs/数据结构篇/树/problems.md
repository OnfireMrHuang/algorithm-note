...
**题目解法:** 题目中的路径解释不一定从根节点出发，可以从二叉树中的任意节点出发并且节点最多出现一次，要求找出其中路径和最大的。

首先我们将视角放到二叉树的一个节点中，该节点的左右子树分别存在一条最大路径，我们这个时候有两种选择:

- 该节点联通左右子树形成一个大的路径，那么此时的路径和为`root.val + left + right`, 这个时候就不能再往上走形成路径了，因为和父节点联通那就分叉了，我称呼这样的路径和叫当前节点的`最大路径和`。
- 该节点与左右子树中最大路径和的一个子树联通，那么此时的路径和为`root.val + max(left, right)`，这个时候还可以继续往上走形成路径, 我称呼这样的路径和叫当前节点的`分支路径和`。

根据以上两点特性，我们在递归遍历二叉树的过程中，每次都计算一下当前节点的最大路径和，然后与全局最大路径和比较，如果大于全局最大路径和，那么就更新全局最大路径和，然后再返回当前节点的分支路径和继续计算。

...
**题目解法:** 该题目与`235`题最大的区别是树为二叉树不是二叉搜索树，说明树的节点值不满足比左子树的任何值大，比右子树的任何值小的约束。针对这种情况，我们可以从根节点出发，递归遍历其左右子树来寻找p或q节点，如果找到了p或q节点，那么就返回该节点，如果没有找到，那么就返回null。当左右子树都返回了非null值时，说明p和q分别在左右子树中，那么当前节点就是p和q的最近公共祖先，如果左右子树返回了null值，说明p和q都不在左右子树中，那么就返回null值。
...
**题目解法:** 前序遍历是一个非常好的序列化方式，因为其依照先排列根节点，再排列左子树，最后再排列右子树的紧凑方式。还记得`105`构造二叉树的题目吗，其中中序也是帮助确认左子树的边界而已，所以如果我们没有中序来辅助恢复二叉树，我们可以通过将`null`也作为一个节点来序列化，这样我们就可以通过左右子树都为null来确定叶子节点边界，从而实现反序列化二叉树。

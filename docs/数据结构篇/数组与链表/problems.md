...
**题目描述**: 给你链表的头结点head，请将其按升序排列, 并返回排序后的链表.
...
**题目解法**: 该题目我们可以采用分治 + 归并的思想来解决，当链表只有一个节点的时候，其默认就是有序的，我们可以对链表不断进行两两拆分，直到拆分
后的链表只有一个节点，然后再将拆分后的链表自底向上进行两两合并，最终合并为一个链表，这个链表就是有序的，图解如下:

![链表排序](../../../resources/images/数据结构/链表排序.png)
